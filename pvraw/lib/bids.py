...
live in ``pvraw.scripts.pvraw``.  Filenames are assembled from the authoritative
...
#: Datatypes pvraw emits into the validated BIDS tree.
...
def validate_suffix(datatype, suffix):
    ...
    if datatype not in DATATYPES:
        raise InvalidApproach(
            f"'{datatype}' is not a BIDS datatype handled by pvraw {DATATYPES}.")
...
def build_prefix(entities, datatype):
    ...
    pvraw appends ``_run-XX``, ``_echo-X`` and the suffix downstream (run indices are
