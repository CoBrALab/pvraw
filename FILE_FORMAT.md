...
The name is generated by ParaVision and its composition is not specified by the manuals; observed
datasets commonly use `<timestamp>_<SubjectName>_<StudyNum>_<ScanNum>`, but a reader must not
parse meaning out of it — use the `subject` file and the Visu study parameters instead.
